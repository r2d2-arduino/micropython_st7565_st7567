"""
ST7565_SPIFB v 0.1.4
Display controller driver (with framebuffer)

Displays: ST7565, ST7567
Connection: SPI
...
class ST7565_SPI( framebuf.FrameBuffer ):
    def __init__( self, spi, cs_pin, dc_pin, rst_pin, rotation = 0, shift = 4 ):
        ...
        if self.rotation & 1:
            ...
            fb_format = framebuf.MONO_HMSB
        else:
            self.width = DISPLAY_WIDTH
            self.height = DISPLAY_HEIGHT
            fb_format = framebuf.MONO_VLSB
    ...
    def draw_text( self, text, x, y ):
        ...
        for char in text:   
            ...
            self.draw_bitmap(glyph[0], x, y, glyph_width, glyph_height)
            x += glyph_width

    @micropython.viper
    def draw_bitmap( self, bitmap, x:int, y:int, width:int, height:int ):
        """ Draw a bitmap (glyph) on display
        Args
        ...
        """
        data   = ptr8(bitmap) #memoryview to bitmap
        ...
        for h in range(height):
            ...
            while bit_len < width:
                ...
                if (byte >> 7) & 1:
                    self.pixel(xpos    , ypos, 1)
                if (byte >> 6) & 1:
                    self.pixel(xpos + 1, ypos, 1)
                if (byte >> 5) & 1:
                    self.pixel(xpos + 2, ypos, 1)
                if (byte >> 4) & 1:
                    self.pixel(xpos + 3, ypos, 1)
                if (byte >> 3) & 1:
                    self.pixel(xpos + 4, ypos, 1)
                if (byte >> 2) & 1:
                    self.pixel(xpos + 5, ypos, 1)
                if (byte >> 1) & 1:
                    self.pixel(xpos + 6, ypos, 1)
                if byte & 1:
                    self.pixel(xpos + 7, ypos, 1)
            ...
            if planes == 1 and depth == 1 and compress == 0: #compress method == uncompressed
                ...
                for h in range(height):
                    ...
                    for w in range(block):
                        bitmap[total_size - 1 - offset - w] = 255 - data[offset + block - 1 - w]

                self.draw_bitmap(bitmap, x, y, width, height)
